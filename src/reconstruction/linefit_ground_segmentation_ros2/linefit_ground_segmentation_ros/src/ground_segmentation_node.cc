...
SegmentationNode::SegmentationNode(const rclcpp::NodeOptions& node_options) : Node("ground_segmentation", node_options)
{
  gravity_aligned_frame_ = this->declare_parameter("gravity_aligned_frame", "");
  ...
  params_.max_fit_error = this->declare_parameter("max_fit_error", params_.max_fit_error);
  ...
  params_.isSensorInverted = this->declare_parameter("isSensorInverted", params_.isSensorInverted);
  ...
  cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      input_topic, rclcpp::SensorDataQoS(), std::bind(&SegmentationNode::scanCallback, this, std::placeholders::_1));
  rclcpp::QoS pub_qos(10);      // 队列深度 10
  pub_qos.reliable();   
  ground_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(ground_topic, pub_qos);
  obstacle_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(obstacle_topic, pub_qos);
  ...
  RCLCPP_INFO(this->get_logger(), "params_.r_min_square: %f", params_.r_min_square);
}
...
void SegmentationNode::scanCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  if (!gravity_aligned_frame_.empty())
  {
    ...
    try
    {
      ...
      tf.rotate(Eigen::Quaterniond(tf_stamped.transform.rotation.w, tf_stamped.transform.rotation.x,
