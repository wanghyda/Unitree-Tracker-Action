...
GroundSegmentation::GroundSegmentation(const GroundSegmentationParams& params)
  ...
  , segments_(params.n_segments,
              Segment(params.n_bins, params.min_slope, params.max_slope, params.max_error_square, params.long_threshold,
...
{
  segments_ = std::vector<Segment>(params_.n_segments,
                                   Segment(params_.n_bins, params_.min_slope, params_.max_slope,
                                           params_.max_error_square, params_.long_threshold, params_.max_long_height,
                                           params_.max_start_height, params_.sensor_height));
}
...
void GroundSegmentation::assignCluster(std::vector<int>* segmentation)
{
  ...
  for (unsigned int i = 0; i < params_.n_threads; ++i)
  {
    ...
    thread_vec[i] = std::thread(&GroundSegmentation::assignClusterThread, this, start_index, end_index, segmentation);
  }
  for (auto it = thread_vec.begin(); it != thread_vec.end(); ++it)
  {
    it->join();
...
void GroundSegmentation::insertionThread(const PointCloud& cloud, const size_t start_index, const size_t end_index)
{
  ...
  for (unsigned int i = start_index; i < end_index; ++i)
  {
    ...
    if (params_.isSensorInverted)
    {
      //雷达倒装，需要修改坐标（通过矩阵变换的方式有点问题，先采用这种方式处理）
      point.y = -point.y;
      point.z = -point.z;
    }
