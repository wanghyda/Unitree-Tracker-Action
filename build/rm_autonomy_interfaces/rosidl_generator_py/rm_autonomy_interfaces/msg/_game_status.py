    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'rm_autonomy_interfaces.msg.GameStatus')
            logger.debug(
...
class GameStatus(metaclass=Metaclass_GameStatus):
    ...
    def __init__(self, **kwargs):
        ...
        self.stage_progress = kwargs.get('stage_progress', int())
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.stage_progress != other.stage_progress:
            return False
    ...
    def stage_progress(self):
        """Message field 'stage_progress'."""
        return self._stage_progress

    @stage_progress.setter
    def stage_progress(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'stage_progress' field must be of type 'int'"
            assert value >= 0 and value < 256, \
                "The 'stage_progress' field must be an unsigned integer in [0, 255]"
